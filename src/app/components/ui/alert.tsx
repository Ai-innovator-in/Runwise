...
const alertVariants = cva(
  "relative w-full rounded-xl border px-4 py-3 text-sm grid has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] grid-cols-[0_1fr] has-[>svg]:gap-x-3 gap-y-0.5 items-start [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current",
  {
    variants: {
      variant: {
        default: "bg-white border-[#005932]/20 text-[#1a1c1b]",
        destructive:
...
function AlertTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "col-start-2 line-clamp-1 min-h-4 font-medium tracking-tight text-[#1a1c1b]",
        className,
      )}
      {...props}
    />
...
function AlertDescription({
  ...
  return (
    <div
      ...
      className={cn(
        "text-[#1a1c1b]/60 col-start-2 grid justify-items-start gap-1 text-sm [&_p]:leading-relaxed",
        className,
      )}
      {...props}
    />
