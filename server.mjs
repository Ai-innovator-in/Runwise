...
function makeInvoicePdf(
  ...
  // Header
  addText(72, 740, businessName, 18, true);
  if (location) {
    addText(72, 722, location, 10);
  }
  addText(72, 700, "INVOICE", 24, true);

  // Invoice details (right-aligned)
  const rightX = 400;
  addText(rightX, 740, `Invoice #: ${invoiceNumber}`, 10);
  addText(rightX, 722, `Date: ${invoiceDate}`, 10);
  addText(rightX, 704, `Due Date: ${dueDate}`, 10);

  // Separator line
  lines.push("q 1 w 72 680 468 0 re S Q");

  // Customer section
  addText(72, 660, "Bill To", 12, true);
  addText(72, 642, customerName, 12);

  // Items table header
  const tableY = 610;
  const colX = [72, 250, 350, 420];
  const colWidths = [178, 100, 70, 70];
  const headers = ["Description", "Quantity", "Unit Price", "Amount"];

  // Draw table header background
  lines.push("q 0.95 0.95 0.95 rg");
  lines.push(`${72} ${tableY - 2} ${468} ${18} re f`);
  lines.push("Q");

  headers.forEach((header, i) => {
    addText(colX[i], tableY, header, 10, true);
  });

  // Draw header bottom line
  lines.push("q 1 w 72 592 468 0 re S Q");

  // Items row
  const rowY = tableY - 22;
  addText(colX[0], rowY, item, 10);
  addText(colX[1], rowY, String(quantity), 10);
  addText(colX[2], rowY, `₦${unitPrice.toLocaleString()}`, 10);
  addText(colX[3], rowY, `₦${subtotal.toLocaleString()}`, 10);

  // Draw row bottom line
  lines.push("q 1 w 72 570 468 0 re S Q");
  ...
  lines.push("q 0.9 0.9 0.9 rg");
  lines.push(`${totalsX - 10} ${balanceY - 4} ${200} ${24} re f`);
  lines.push("Q");
  ...
  // Footer
  const footerY = 100;
  addText(72, footerY, "Thank you for your business!", 10);
  addText(72, footerY - 18, "Generated by MarketOS Offline", 8);
